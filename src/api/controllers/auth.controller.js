...
class AuthController {
    ...
    login = async (req, res, next) => {
        try {
            let { name, password } = req.body; // 🔄 Đổi sang let để xử lý dữ liệu thô

            // 🧹 CHỦ ĐỘNG LÀM SẠCH: Loại bỏ khoảng trắng thừa đầu/cuối của chuỗi nhập vào
            if (name && typeof name === 'string') {
                name = name.trim();
            }

            // 🛑 CHẶN EDGE CASE 8.3: Nếu tên chỉ toàn dấu cách (sau khi trim bị rỗng), chặn ngay lập tức bằng mã 400
            if (!name || name === '') {
                return responseBuilder.error(res, 'Tên đăng nhập không được để trống hoặc chỉ chứa khoảng trắng!', 400);
            }
            ...
            // Chuyển tiếp xuống Service để kiểm tra mật khẩu hợp lệ (truyền name đã được trim sạch)
